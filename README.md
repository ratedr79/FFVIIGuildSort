...
5. **Save / Load builds.** Use **Save** (in the top action row) to name and store the current team in your browser; **Load** opens a list of saved builds to restore one (it re-scores on load). Loading over an in-progress build asks for confirmation first, and saving over an existing name prompts to overwrite. Each saved build can be removed from the Load list via its delete button, with a confirmation prompt. Saved builds are stored locally (`itb-saved-teams-v1`) and hold the gear/battle selections only — your inventory and Character Stats are applied fresh on load.
