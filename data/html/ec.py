...
HTML_FILE = "c:\\temp\\Final_Fantasy_VII_Ever_Crisis_weapons.html"
...
for header in soup.find_all(["h3"]):
    ...
    if "Weapon" not in headers or "Reinforcement Abilities" not in headers:
        continue

    heading = next_table.parent.parent.find_previous_sibling("h3")
    char_heading = heading.get_text(strip=True)
    ...
    weapon_index = headers.index("Weapon")
...
# Save JSON
with open("c:\\temp\\ever_crisis_weapons.json", "w", encoding="utf-8") as f:
    json.dump(weapons_data, f, ensure_ascii=False, indent=2)

print(f"Generated ever_crisis_weapons.json with {len(weapons_data)} entries.")