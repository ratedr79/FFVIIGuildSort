...
### Added
- Player Power Analyzer V2 **By Boss** battle-context mode (Phase 2 of the boss-driven analyzer — see `docs/boss-selection-and-sigils-design.md`). A **Manual / By Boss** toggle: in By-Boss mode, search and pick an enemy and the analyzer infers the battle context, shown in an editable summary and prefilled into the existing controls (all overridable). `BossBattleContextDeriver` (pure/static, unit-tested) derives from `EnemyCatalog.GetEnemyDetails`: **Weakness** = most-negative element resistance; **Damage type** = PDEF/MDEF comparison, overridden by def-down-resist asymmetry (resists PDEF Down but not MDEF Down → Magical), overridden by a description hint ("…magic abilities are effective"); **Scenario** = Single Enemy; **Required sigils** = the boss's break sigils; **Bonus sigils** = its damage sigils. Page-model handlers `OnGetBossSearch` (picker) + `OnGetBossContext` (derive). **Damage-sigil bonus (2b):** teams covering a boss's damage sigils get a small gated score nudge (+1.5%/covered sigil, `BonusSigils` on the request); empty in Manual mode → byte-identical analyzer repro. Verified: deriver unit tests; Titan EX 3 (enemy 2552006) derives Wind / Magical / Diamond-required / Circle-bonus; UI prefill + description-hint path. (Fixed a real prefill bug found via screenshot: `Html.GetEnumSelectList` uses integer option *values* with the enum name as *text*, so the weakness/damage-type/scenario selects must be matched by option text, not value.)
- Player Power Analyzer V2 **Required Sigils** filter (Advanced filters; Phase 1 of the boss-driven analyzer — see `docs/boss-selection-and-sigils-design.md`). Pick any of ◯ Circle / △ Triangle / ✕ Cross / ◊ Diamond; a team qualifies only if, for **each** selected sigil, some character **covers** it: a **main-hand** sigil-boost materia slot for Circle/Triangle/Cross, or a **main- or off-hand** weapon ability command sigil for Diamond (Diamond has no materia slot). Implemented as a **team-level candidate filter** in `Analyze` (after team build — not a seed constraint, so it avoids the monotonicity-sensitive stage), reading `item.Sigils` (`Source` = Materia/Command) by item id. No match → `FailureReason` names the unmet sigil(s). `RequiredSigils` added to `PlayerPowerAnalyzerV2Request` + the page model; default empty → byte-identical analyzer repro. Pairs with the Enemy Stats Battle Sigils so a player can read a boss's break sigils and require them here. Integration test added (real weapon data: Triangle-covered team passes; uncovered sigil → no team + named reason); V2 service suite + repro green.
- Enemy Stats detail now shows a **Battle Sigils** section, with **Break** (blue) and **Damage** (amber) sigils differentiated. Break sigils come from `AutoBattleNotesRel.json` (the main gimmick sigil[s] to break); damage sigils come from `AutoBattleNotesDamageRel.json` (sigils that add bonus damage after the break) — e.g. Train of Titan EX 3 shows Break ◊ Diamond + Damage ◯ Circle, matching the in-game UI. `EnemyCatalog` resolves both via the existing `Battle → WaveGroup → EnemyGroup → Enemy` join (`BuildBattleSigilsByEnemy`, `SkillNotesId → sigil`: 1101 Circle / 2101 Triangle / 3101 Cross / 4101 Diamond); `EnemyDetailView` gained `BattleSigils` + `BattleDamageSigils`. Sigils are unioned per enemy across its battles and shown only when present. Notes: the source is the auto-battle note relations (a strong proxy for the battle's sigil gimmick, not an explicit "required" flag), and coverage is broad (~982 enemies). Verified live for Train of Titan EX 3 (enemy 2552006). (Caught a Bootstrap 5.1 badge bug along the way — `text-bg-primary` is 5.2+ and rendered white-on-white; switched to `bg-primary text-white` / `bg-warning text-dark`.)
...
### Added
- Player Inventory **Find & Manage → "Sort by"** dropdown (6 options; default Default Ascending): Default Ascending/Descending (character id, then item id), Item Name Ascending/Descending, and Char. & Item Name Ascending/Descending. Weapons and costumes sort as one combined list (the Item type filter still narrows the list first); a numeric-aware item-id is the final tiebreaker for deterministic ordering. The Char. & Item Name **Descending** variant keeps character names ascending and reverses only the item name (by request). Persisted in the inventory state's `filters.sortBy` (whitelisted; survives reload). "Clear filters" leaves the sort untouched (treated as a view preference, not a filter). Replaced the fixed `compareInventoryItems` (character id → equipment type → name → id) with a `state.filters.sortBy`-driven comparator map. Verified end-to-end via Playwright (6 options, ordering per spec incl. the asymmetric descending variant, and persistence).
