...
- **Character Stats** (panel above the catalog + "Enter Character Stats" modal): records per-character stat inputs so future damage estimates can use real attack stats. Stored separately in browser-local `player-character-stats-v1`.
  ...
  - Data entry only for now; Phase 2 wires it into the Interactive Team Builder, where total PATK/MATK = `floor((intrinsic + weapon stats + branding) × (1 + Highwind%))` feeds the Damage Calc. Weapon **branding** (RNG per-weapon PATK/MATK) is not recorded and is the main expected source of residual error. A later step could auto-default the streams to their role/growth-board max.
