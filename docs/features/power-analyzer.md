...
**Export guilds CSV** exports the displayed result rather than re-running: `OnGetExportGuilds(resultJobId)` formats the CSV from the completed job's bundle (`BuildGuildAssignmentsCsv`), so it matches the on-screen engine (V2 or legacy) and is sub-second. The button only appears after an analysis has been run.
