...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Extract and download FFVIIEC ultimate weapon images from a saved Fandom page.")
    parser.add_argument(
        "--source-file",
        type=Path,
        default=script_dir / "ultimateweapons.html",
        help="Path to the saved ultimateweapons.html file. Defaults to this script's sibling ultimateweapons.html.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=script_dir / "downloaded_ultimate_weapon_images",
...
def main() -> int:
    ...
    if not entries:
        print(
            "No Fandom ultimate weapon image rows were found. Make sure ultimateweapons.html is saved and still contains the weapon table with static.wikia.nocookie.net image links.",
            file=sys.stderr,
        )
        return 2

    print(f"Found {len(entries)} unique ultimate weapon image URLs")
